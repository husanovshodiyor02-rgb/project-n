...
//     try {
//       console.log("So'rov yuborilmoqda:", `${API_URL}/api/auth/sign-in`);
...
//         router.push("/admin");
//       } else {
//         setError("Login muvaffaqiyatsiz. Iltimos qayta urining.");
//       }
//     } catch (err: any) {
...
//       <Card className="w-100 shadow-xl border-none dark:bg-slate-900 dark:border dark:border-slate-800">
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // MANTIQ (Birinchi koddan olindi - Middleware ishlashi uchun)
      const data = res.data;
      // Har xil backend javob turlari uchun tekshiruv
      const token = data.token || data.data?.token || data.accessToken;
      const role = data.role || data.data?.role || data.user?.role || "admin";
      const user = data.user || data.data?.user || data.data;

      if (token) {
        // 1. COOKIE GA SAQLASH (Middleware uchun zarur)
        document.cookie = `token=${token}; path=/; max-age=86400; SameSite=Lax`;
        document.cookie = `role=${role}; path=/; max-age=86400; SameSite=Lax`;

        // 2. LOCALSTORAGE GA SAQLASH (Client side uchun)
        ...
        // 3. Admin sahifasiga yo'naltirish
        router.push("/admin");
      } else {
