...
export default function AdminLayout({
  ...
  // User state ni to'liq ma'lumotlar bilan saqlash uchun o'zgartirdik
  ...
  // 1. User ma'lumotlarini bazadan o'qish funksiyasi
  const fetchUserData = async () => {
    ...
    try {
      // BAZADAN ENG SO'NGGI MA'LUMOTNI OLISH
      ...
      setUser({
        ...
        avatar: data.image || data.avatar || "/placeholder-avatar.png",
      });
    } catch (error) {
      console.error("Layoutda userni yuklashda xato:", error);
      // Agar token eskirgan bo'lsa login page ga otish
  ...
  useEffect(() => {
    // Sahifa ochilganda ma'lumotni o'qish
    fetchUserData();

    // CUSTOM EVENT LISTENER: ProfilePage dan keladigan signalni kutish
    window.addEventListener("profileUpdated", fetchUserData);

    // Komponent o'chganda listenerni tozalash
    return () => {
            ...
            <div className="flex items-center gap-3 border-l pl-4 dark:border-slate-700">
              <div className="text-right hidden md:block">
                <p className="text-sm font-medium text-slate-900 dark:text-white">
                  {/* ISM VA FAMILIYA REAL VAQTDA */}
                  {user?.first_name || "Yuklanmoqda..."} {user?.last_name || ""}
                </p>
              ...
              <Avatar>
                ...
                <AvatarImage
                  src={user?.avatar}
                  onError={(e: any) => {
                    if (!e.target.src.includes("placeholder-avatar.png")) {
